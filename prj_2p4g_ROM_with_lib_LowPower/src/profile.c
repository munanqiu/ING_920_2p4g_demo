...
static void ble_adv_stopped_handler(void){
    ...
    // ble_switch_to_2p4g_check();
    btstack_reset();
    // platform_set_timer(adv_stop_switch, 3200);
}
...
static void setup_adv(void)
{
    static const bd_addr_t rand_addr = { 0xDD, 0x28, 0x1C, 0xD8, 0x69, 0x69 };
...
extern void us_timer_test_init(void);
